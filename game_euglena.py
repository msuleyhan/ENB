...
# Timing and states
initial_welcome_time = 1  # seconds
initial_detection_time = 1  # seconds
game_duration = 3  # seconds per zone
result_display_duration = 1  # seconds to display results
total_game_duration = 9  # Total game duration (for 3 zones)
start_time = time.time()
last_state_change = time.time()

initial_keypoints = []
paths = {}
game_state = 'welcome'  # Can be 'welcome', 'detecting', 'tracking', 'show_result'
target_region = None
region_count = 0
completed_zones = 0
...
while True:
    ...
    if game_state == 'welcome':
        if elapsed_time <= initial_welcome_time:
            # Display a black screen with "Welcome to the game"
            frame = np.zeros((500, 500, 3), dtype=np.uint8)
            cv2.putText(frame, "Welcome to the game", (50, 250), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
            ...
            start_time = time.time()  # Reset start_time for 'detecting'
    ...
    if game_state == 'detecting' and elapsed_time <= initial_detection_time:
        ...
        cv2.putText(frame, "Goodbye, game over", (50, 250), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, score_text, (score_x, score_y), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
        cv2.imshow('Bacteria Tracking Game', frame)
